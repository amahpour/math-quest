...
function GQGameOver({ won, bananas, gems, level, onReset, onHome }) {
  return (
    <div style={{ position: 'absolute', inset: 0, background: won ? 'radial-gradient(ellipse, #3F1F7A 0%, #1B0A3D 100%)' : 'radial-gradient(ellipse, #5A1B1B 0%, #1A0505 100%)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 30, animation: 'gq-fadein .4s' }}>
      <div style={{ textAlign: 'center', maxWidth: 420 }}>
        ...
        <div style={{ color: 'white', fontSize: 16, margin: '14px 0 22px', opacity: 0.9 }}>
          {won ? `You conquered all 3 levels — Forest, City, Volcano! 🌋` : `Your character ran out of steam on level ${level || 1}.`}
        </div>
        <div style={{ display: 'flex', justifyContent: 'center', gap: 24, marginBottom: 24, color: 'white' }}>
          ...
          <div><div style={{ fontSize: 28 }}>🗺️</div><div style={{ fontFamily: '"Fredoka One", sans-serif', fontSize: 22 }}>{level || 1}/3</div></div>
        </div>
...
// Character portrait — cartoon head with selectable palette.
function GQCharacterFace({ character, size = 96 }) {
  const S = size;
  return (
    <svg width={S} height={S} viewBox="0 0 100 100">
      {/* hat */}
      <ellipse cx="50" cy="30" rx="32" ry="12" fill={character.hat} stroke="#2a1010" strokeWidth="2" />
      <ellipse cx="50" cy="22" rx="24" ry="14" fill={character.hat} stroke="#2a1010" strokeWidth="2" />
      {/* face */}
      <ellipse cx="50" cy="55" rx="26" ry="28" fill={character.skin} stroke="#2a1010" strokeWidth="2" />
      {/* eyes */}
      <circle cx="41" cy="54" r="3" fill="#2a1010" />
      <circle cx="59" cy="54" r="3" fill="#2a1010" />
      <circle cx="42" cy="53" r="1" fill="#fff" />
      <circle cx="60" cy="53" r="1" fill="#fff" />
      {/* smile */}
      <path d="M40 66 Q50 74 60 66" stroke="#2a1010" strokeWidth="2.5" fill="none" strokeLinecap="round" />
      {/* shirt */}
      <path d="M22 92 Q22 78 50 78 Q78 78 78 92 L78 100 L22 100 Z" fill={character.shirt} stroke="#2a1010" strokeWidth="2" />
    </svg>
  );
}

// Character picker — below HUD z-index so Menu button stays clickable.
function GQCharacterPicker({ onPick }) {
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'rgba(6,6,71,0.88)', backdropFilter: 'blur(4px)', zIndex: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, animation: 'gq-fadein .3s ease-out' }}>
      <div style={{ background: 'linear-gradient(180deg, #1A2AC7 0%, #060647 100%)', border: '3px solid #FFCC00', borderRadius: 18, padding: 26, maxWidth: 540, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.6), 0 0 40px rgba(255,204,0,0.2)' }}>
        <div style={{ fontFamily: '"Bebas Neue", sans-serif', fontSize: 32, color: '#FFCC00', letterSpacing: 3, textAlign: 'center' }}>PICK YOUR HERO</div>
        <div style={{ textAlign: 'center', color: 'rgba(255,255,255,0.75)', fontSize: 13, marginBottom: 18 }}>Who's adventuring through the forest, city, and volcano?</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
          {window.GQ_CHARACTERS.map((c) => (
            <button key={c.id} onClick={() => onPick(c)} style={{
              background: 'rgba(255,255,255,0.08)',
              border: '2px solid rgba(255,204,0,0.4)',
              borderRadius: 12, padding: '12px 8px', cursor: 'pointer',
              display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6,
              fontFamily: '"Fredoka One", sans-serif', color: 'white', transition: 'transform .15s, border-color .15s',
            }} onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-3px)'; e.currentTarget.style.borderColor = '#FFCC00'; }}
               onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.borderColor = 'rgba(255,204,0,0.4)'; }}>
              <GQCharacterFace character={c} size={84} />
              <div style={{ fontSize: 18, letterSpacing: 1 }}>{c.name}</div>
            </button>
          ))}
        </div>
      </div>
...
// Energy quiz — same look as GQMCQuiz but awards +1000 energy on first correct answer.
function GQEnergyQuiz({ onResult }) {
  const pickRandom = () => window.MC_QUESTIONS[Math.floor(Math.random() * window.MC_QUESTIONS.length)];
  const [q, setQ] = React.useState(pickRandom);
  const [picked, setPicked] = React.useState(null);
  const [locked, setLocked] = React.useState(false);
  ...
  const pick = (i) => {
    ...
    if (correct) {
      ...
      setTimeout(() => {
        let next = pickRandom();
        if (window.MC_QUESTIONS.length > 1) while (next === q) next = pickRandom();
        setQ(next); setPicked(null); setLocked(false);
        setAttempts((a) => a + 1);
      }, 1600);
    }
  };
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,50,30,0.88)', backdropFilter: 'blur(6px)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, animation: 'gq-fadein .25s ease-out' }}>
      <div style={{ background: 'linear-gradient(180deg, #0F5F36 0%, #042818 100%)', border: '3px solid #7EE8A6', borderRadius: 16, padding: 26, maxWidth: 560, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.6), 0 0 40px rgba(126,232,166,0.3)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
          <div style={{ fontFamily: '"Bebas Neue", sans-serif', fontSize: 24, color: '#7EE8A6', letterSpacing: 2 }}>⚡ ENERGY CHALLENGE</div>
          <div style={{ fontSize: 11, color: 'rgba(126,232,166,0.8)', letterSpacing: 1.5 }}>{q.topic.toUpperCase()}</div>
        </div>
        <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.75)', marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span>🔋 Get it right → <strong style={{ color: '#7EE8A6' }}>+1000 ⚡ energy</strong> · keep trying on wrong answers</span>
          {attempts > 0 && <span style={{ color: '#FF6B6B', fontWeight: 700 }}>Try #{attempts + 1}</span>}
        </div>
        <div style={{ background: 'rgba(0,0,0,0.3)', padding: '20px', borderRadius: 10, fontFamily: '"Korinna", Georgia, serif', fontSize: 30, fontWeight: 700, color: 'white', textAlign: 'center', marginBottom: 18, textShadow: '2px 2px 0 #000', border: '1px solid rgba(126,232,166,0.3)' }}>
          <window.MQMath src={q.q} />
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          {q.choices.map((c, i) => {
            ...
            return (
              <button key={i} onClick={() => pick(i)} disabled={locked} style={{
                background: state === 'right' ? '#43A047' : state === 'wrong' ? '#E53935' : state === 'reveal' ? 'rgba(67,160,71,0.3)' : state === 'dim' ? 'rgba(255,255,255,0.06)' : 'rgba(255,255,255,0.1)',
                border: state === 'right' || state === 'wrong' ? '2px solid white' : state === 'reveal' ? '2px dashed #43A047' : '2px solid rgba(126,232,166,0.4)',
                color: state === 'dim' ? 'rgba(255,255,255,0.5)' : 'white',
                padding: '14px 12px', borderRadius: 8, fontSize: 17, fontWeight: 700,
                cursor: locked ? 'default' : 'pointer', fontFamily: '"Bebas Neue", sans-serif', letterSpacing: 1,
                transition: 'all .15s', transform: state === 'right' ? 'scale(1.04)' : 'scale(1)',
        ...
        {locked && picked !== null && (
          <div style={{ marginTop: 14, textAlign: 'center', fontFamily: '"Bebas Neue", sans-serif', fontSize: 24, letterSpacing: 2, color: picked === q.answer ? '#4ADE80' : '#FF6B6B' }}>
            {picked === q.answer ? '✓ CORRECT · +1000 ⚡' : '✗ WRONG — new question coming…'}
          </div>
        )}
      </div>
    </div>
...
// Bus destination picker — 5 gems per ride.
function GQBusPicker({ stops, currentStop, gems, onGo, onCancel }) {
  const cost = 5;
  const canAfford = gems >= cost;
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'rgba(20,10,50,0.85)', backdropFilter: 'blur(6px)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'linear-gradient(180deg, #FFCC00 0%, #B38600 100%)', border: '4px solid #3a2600', borderRadius: 14, padding: 22, maxWidth: 440, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.6)' }}>
        <div style={{ fontFamily: '"Fredoka One", sans-serif', fontSize: 28, color: '#3a2600', textAlign: 'center', letterSpacing: 1 }}>🚌 BUS STOP</div>
        <div style={{ textAlign: 'center', color: '#3a2600', fontSize: 13, marginBottom: 14, fontWeight: 700 }}>
          Currently at: <strong>{currentStop.label}</strong> · Ride = 💎 {cost}
        </div>
        <div style={{ display: 'grid', gap: 8, marginBottom: 14 }}>
          {stops.map((s) => (
            <button key={s.id} onClick={() => onGo(s)} disabled={!canAfford} style={{
              background: canAfford ? 'rgba(255,255,255,0.95)' : 'rgba(255,255,255,0.5)',
              border: '2px solid #3a2600',
              color: '#3a2600', padding: '12px 14px', borderRadius: 8,
              cursor: canAfford ? 'pointer' : 'not-allowed',
              fontFamily: '"Fredoka One", sans-serif', fontSize: 17, letterSpacing: 0.5,
              display: 'flex', justifyContent: 'space-between', alignItems: 'center',
            }}>
              <span>🚏 {s.label}</span>
              <span style={{ fontSize: 13 }}>💎 {cost}</span>
            </button>
          ))}
        </div>
        {!canAfford && (
          <div style={{ textAlign: 'center', color: '#7a2a0a', fontSize: 13, fontFamily: '"Fredoka One", sans-serif', marginBottom: 10 }}>Need 💎 {cost} — collect more gems first!</div>
        )}
        <button onClick={onCancel} style={{ width: '100%', background: 'transparent', border: '1.5px solid #3a2600', color: '#3a2600', padding: '10px', borderRadius: 8, cursor: 'pointer', fontFamily: '"Fredoka One", sans-serif', fontSize: 14, letterSpacing: 1 }}>Walk away</button>
      </div>
    </div>
  );
}

Object.assign(window, { GQMCQuiz, GQWagerPicker, GQShop, GQGameOver, GQCharacterPicker, GQEnergyQuiz, GQBusPicker, GQCharacterFace });
